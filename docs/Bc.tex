...
Pohyb těles po eliptických drahách je v~aplikaci řešen především na základě Keplerových zákonů. V~této kapitole jsou uvedeny kompletní vzorce, v~aplikaci jsou ale samozřejmě výpočty optimalizované a~konstantní mezivýpočty jsou u~každého tělesa provedeny pouze jednou a~uloženy do pomocného objektu.

\subsubsection{Inicializace pozice}

Při načtení scény je nejprve nutno inicializovat pozici všech těles. V~databázi je u~každého tělesa uložený datum, v němž těleso prošlo periapsidou. Následně dojde k~výpočtu rozdílu dnešního data a~data průchodu periapsidou. Dalším krokem je výpočet excentrické anomálie E~--~úhlu, jenž svírá spojnice periapsidy se středem orbity a~spojnice pozice tělesa na pomyslné kruhové orbitě se středem orbity. Toho lze docílit pomocí Keplerovy rovnice.~\cite{kleczek}
...
Vyjádření excentrické anomálie E~z~Keplerovy rovnice ovšem není možné. Pro výpočet byla proto zvolena Newtonova iterační metoda, pomocí níž se výsledek jednotlivými kroky stále zpřesňuje, až dokud přesnost nedosáhne požadované hranice.
...
Z~vypočtené excentrické anomálie je možné získat skutečnou anomálii \textit{v} -- úhel mezi spojnicí těžiště a~periapsidy a~spojnicí těžiště a~tělesa.

$$v = 2 * arctg\Bigg(\sqrt{\frac{1 + \epsilon}{1 - \epsilon}} * tg\bigg(\frac{E}{2}\bigg)\Bigg)$$
\begin{center}
\textit{E -- excentrická anomálie, $\epsilon$ -- absolutní excentricita}~\cite{michalrepik}
\end{center}

Pro další účely je ovšem třeba úhel mezi spojnicemi středu a~periapsidy a~středu a~tělesa. K tomu poslouží jednoduchá úprava // TODO.

\subsubsection{Inicializace rotace}

Inicializace rotace je mnohem elementárnější problém, než inicializace pozice, protože se počítá s~tím, že rychlost rotace tělesa je (v~rámci stejného místa na povrchu) konstantní. V~databázi je uložen úhel rotace tělesa kolem své osy k~1.~1.~2019 v~0.00:00. Následně dojde ke zjištění času, který od této doby uplynul a~k~výpočtu úhlu, o~něhož se stihlo za tento rozdíl časů těleso pootočit kolem své osy.

$$\alpha = \alpha\textsubscript{0} + \Bigg(\frac{2 * \pi}{T} * \bigg((t - t\textsubscript{0})~mod~T\bigg)\Bigg)$$
\begin{center}
\textit{$\alpha$ -- aktuální úhel rotace, $\alpha\textsubscript{0}$ -- inicializační úhel rotace, T -- perioda oběhu, t -- aktuální čas, t\textsubscript{0} -- inicializační čas}
\end{center}
