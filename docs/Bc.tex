...
\subsubsection{Pixel, Voxel a Shader}
...
\textit{WebGL} je verze \textit{OpenGL} určená pro web. Jedná se o~javascriptové rozhraní pro zobrazování nativní 2D a~3D grafiky.~\cite{graphic} Program pracující s~\textit{WebGL} se skládá z~javascriptového řídícího kódu a~z~tzv. \textit{shaderu}.
...
Oba programy mají vlastní funkci \code{main}, která je vstupním bodem. \textit{Vertex shader} se provede nad každým vrcholem grafického primitiva. Vstupem i~výstupem je vždy jeden vrchol. Nelze tedy přidávat nebo odebírat vrcholy. Je možné pouze uplatňovat různé operace, např. transformace, na již existující vrcholy. \textit{Fragment shader}, někdy také \textit{pixel shader}, je prováděn nad každým pixelem grafického primitiva. Vedle těchto programů jsou zde obsaženy i~některé původní fáze z~\textit{fixed-function pipeline}.~\cite{graphic}
...
\begin{itemize}
\item Hmotnost obíhajících těles je zanedbána. Centrální těleso tedy není ovlivněno tělesy obíhajícími. Oběh je počítán na základě 1. a~2. Keplerova zákona a není třeba řešit vzájemné gravitační ovlivňování těles.
...
\begin{center}
\textit{G – gravitační konstanta, m – hmotnost tělesa, r – poloměr tělesa}~\cite{kleczek}
\end{center}
...
Na stránce detailu uživatele jsou zobrazeny informace, které o sobě uživatel zveřejnil. Je zde možné tak vidět např. pohlaví, věk, bydliště, profilový text nebo možnosti kontaktu. Mimo to se zde zobrazují i~uživatelské statistiky, jež obsahují následující informace:

\begin{itemize}
\item Jaká tělesa upravil nebo přidal do databáze,
\item Jaké události těles upravil nebo přidal do databáze,
...
Na této stránce může uživatel změnit některé z~informací o~něm. Konkrétně se jedná o~pohlaví, věk, bydliště, profilový text, veřejný email a~adresu webové stránky nebo profilu na některé ze sociálních sítí.
