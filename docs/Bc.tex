...
Pro popis geometrických, astronomických či jiných fyzikálních vlastností těles a~jejich eliptických orbit jsou určeny termíny. V~této kapitole je stručně popsán význam některých z nich.~\cite{kleczek}
...
\begin{center}
\textit{$\varrho$ -- hustota, M -- hmotnost tělesa, V -- objem tělesa, f~--~zploštění tělesa, r -- poloměr tělesa, $v_{max}$ -- max. rychlost, $\mu$ -- gravitační parametr, $d_{min}$, periapsida}
\end{center}
...
Dalším důležitým údajem je aktuální \textbf{vzdálenost tělesa od těžiště}. Celý výpočet uskuteční knihovní metoda v THREE.js \code{bodyPosition.distanceTo(parentPosition)}.

Pro aktualizaci pozice zbývá vypočítat \textbf{aktuální rychlost} oběhu, \textbf{uraženou dráhu} a~\textbf{změnu úhlu} od posledního vykreslení. Problém elipsy je zjednodušen na problém trojúhelníka za cenu zanedbatelné nepřesnosti. Jednotlivé nepřesnosti se díky kontrolním mechanismům přitom sčítají pouze v~rámci jedné periody oběhu. Poté je nepřesnost vyrovnána.

\vspace*{-0.5cm}
$$v = \sqrt{\mu * \frac{2}{d} - \frac{1}{a}}~~~~~~~~~~\Delta s = v * \Delta t~~~~~~~~~~\omega = \frac{v}{O}$$
\begin{center}
\textit{v -- okamžitá rychlost, $\mu$ -- gravitační parametr, d -- aktuální vzdálenost od těžiště, a -- velká poloosa}
\end{center}

Po přičtení úhlu oběhu tělesa lze získat nové souřadnice tělesa pomocí metody\break \code{orbit.getPoint(angle)}.

\begin{figure}[H]
\begin{center}
\includegraphics[width=300pt]{Images/DeltaAngle.png}
\caption[Výpočet nové pozice tělesa]{Výpočet nové pozice tělesa \footnotemark[1]}
\end{center}
\end{figure}
...
Po spuštění databáze je nutno do souboru \code{/src/Constants/Config.ts} v~aplikaci doplnit položky \code{database.username} a~\code{database.password}. Dále je třeba v~terminálu z~kořenového adresáře této aplikace spustit~skript, který do databáze vloží základní data -- několik těles a~především uživatele s~administrátorskými právy (email: \textit{admin@universis.com}, heslo: \textit{aDM!n7paSsw0RD}).

\lstinputlisting[caption={Vložení inicializačních dat do databáze}, language={JavaScript}]{SourceCodes/InsertScript.js}

Po úspěšné instalaci musí dojít ke~stažení externích balíčků \textit{NPM} pro server i~pro klienta, transpilaci zdrojových kódů do nativního \textit{JavaScriptu} a~spuštění serveru. Pro všechny tyto kroky je v~aplikaci předpřipraven příkaz pro unixové systémy (testováno na \textit{Ubuntu~18}, \textit{Debian~9} a~\textit{macOS Mojave}) a~pro Windows 10.

\lstinputlisting[caption={Sestavení a~spuštění aplikace}, language={JavaScript}]{SourceCodes/Installation.js}
