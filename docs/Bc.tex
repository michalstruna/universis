...
\lstdefinelanguage{JavaScript}{
  keywords={const, let, class, extends, export, default, public, static, typeof, new, true, false, catch, function, return, null, catch, switch, var, if, in, while, do, else, case, break},
...
Program je vytvořen v~\textit{C++} a \textit{Lua}, což mu dovoluje výkon počítače využívat podstatně lépe, než webová aplikace. Navzdory tomu je nutno tento program nejdříve stáhnout, což může některé uživatele odradit. Je však dostupný jak pro \textit{Windows}, tak i~pro \textit{Linux} a~\textit{macOS}. Neobsahuje ovšem češtinu. 
...
Obsahuje velice rozsáhlou databázi obsahující v~základní instalaci necelý milion těles. Je však možné si dodatečně nainstalovat katalogy s~více jak 200 miliony tělesy. U každého tělesa je možné se zobrazit stručné informace, především co se jejich polohy a~vzdálenosti od Země týče.
...
\footnotetext[1]{Fotografie pořízena z \url{https://space.jpl.nasa.gov/}.}
...
\textit{TypeScript} je programovací jazyk vyvinutý firmou \textit{Microsoft}. Jedná se o~nádstavbu jazyka \textit{JavaScript}, která přidává statické typování a~další vlastnosti objektového programování. Kód napsaný v~jazyce \textit{JavaScript} je kompatibilní s~\textit{typescriptovým} kódem. Pro kompatibilitu v~prohlížečích je však nutné veškerý \textit{TypeScript} transpilovat do \textit{javascriptového} kódu.
...
\lstinputlisting[caption={Porovnání vykreslení komponenty ve standardním JavaScriptu a~v~JSX}, language={JavaScript}]{SourceCodes/JSX.ts}
...
\lstinputlisting[caption={Konfigurace nástroje Webpack v souboru webpack.config.json}, language={JavaScript}]{SourceCodes/Webpack.ts}.
...
\lstinputlisting[caption={Správné a~nesprávné použití importu souboru z~modulu}, language={JavaScript}]{SourceCodes/ImportFromModule.ts}


\subsection{Klientská část}

Klientská část aplikace dodržuje architekturu \textit{Redux}. Všechen stav aplikace je na jednom místě v tzv. \textit{store}, odkud ho mohou číst všechny komponenty, jež jsou na \textit{store} napojené. Dochází tak k~eliminování případů, kdy jednotlivé komponenty komunikují se svými rodičovskými nebo vnořenými komponentami, což je ve větších aplikacích nepřehledné.
...
\lstinputlisting[caption={Ukázka architektury Redux}, language={JavaScript}]{SourceCodes/Redux.ts}

\subsubsection{Akce}

Vytvářet akce manuálně je zdlouhavé, zejména pokud jde o~asynchronní akce. Pro správnou funkcionalitu \textit{UI} je totiž při asynchronním požadavku třeba zaznamenat následující stavy:
...
V~kódu pak použití asynchronní akce vypadá následovně. Automaticky je zajištěno sledování průběhu požadavku a~je tudíž zcela lehce možné zobrazit např. načítací animaci.

\lstinputlisting[caption={Redux akce za využití vlastní knihovny}, language={JavaScript}]{SourceCodes/ReduxAction.ts}

\subsubsection{Reducer}

Reducer je běžně funkce, která obsahuje často rozsáhlý přepínač s~výčtem všech akcí a~jejich modifikací stavu aplikace. Díky vlastní knihovně to ale není nutné. Stačí uvést pole všech akcí a~případně i~počáteční stav \textit{store}.

\lstinputlisting[caption={Redux reducer za využití vlastní knihovny}, language={JavaScript}]{SourceCodes/ReduxReducer.ts}

\subsubsection{3D grafika}

Před vykreslováním těles ve vesmíru je nutné ze serveru nejdříve získat data k~těmto tělesům. Následovně je použita třída \textit{BodyFactory}, která z datových objektů těles vytvoří kontejnery obsahující mimo jiné i~3D útvary tělesa a~jeho orbity. Pro práci s~3D grafikou byla v~rámci projektu vytvořena vlastní knihovna \textit{Scene} v modulu \textit{Utils}.

\lstinputlisting[caption={Práce s vlastní knihovnou pro 3D grafiku}, language={JavaScript}]{SourceCodes/Scene.ts}

Pro to, aby byla scéna interaktivní a~měnila se v~čase je třeba v~parametrech definovat funkce \textit{onClick} a~\textit{onRender}. První ze zmíněných proběhne pokaždé, když je kliknuto na těleso. Druhá funkce probíhá při každém průběhu vykreslovací smyčky (v~ideálním případě 60krát za sekundu).
...
\lstinputlisting[caption={Definice cesty v REST~API}, language={JavaScript}]{SourceCodes/Route.ts}

Dokumentace je zpřístupněna na lokální adrese \textit{/api-docs}. Je možné se zde informovat o~všech možných \textit{HTTP} požadavcích nebo je s~nastavenými parametry nasimulovat.
...
\lstinputlisting[caption={Zpracování HTTP požadavku}, language={JavaScript}]{SourceCodes/RouteUtils.ts}

Modelové třídy většiny entit není nutné psát individuálně. Všechny mají za úkol poskytnout \textit{CRUD operace} nad danou entitou (např. \textit{BodyModel} nad tělesy). Jediné, co se liší, je název databázové kolekce, vybrané sloupce při výpisu detailu entity a~při výpisu všech entit, popř. další parametry. Veškerá logika komunikace s~databází byla ukryta do třídy EntityModel. Většina dalších modelů je pak pouhou instancí této třídy s~konkrétními parametry.

\lstinputlisting[caption={Automatizovaná tvorba modelových tříd}, language={JavaScript}]{SourceCodes/Model.ts}
...
\lstinputlisting[caption={Připojení k databázi v~aplikaci}, language={JavaScript}]{SourceCodes/DatabaseConnection.ts}
...
\lstinputlisting[caption={Vytvoření databázového schématu}, language={JavaScript}]{SourceCodes/Schema.ts}
...
\lstinputlisting[caption={Vytvoření a~použití databázového pluginu}, language={JavaScript}]{SourceCodes/Hash.ts}
...
\lstinputlisting[caption={CRUD operace nad kolekcí uživatelů}, language={JavaScript}]{SourceCodes/Database.ts}
...
\lstinputlisting[caption={Hashování hesel v Mongoose schématu}, language={JavaScript}]{SourceCodes/Hash.ts}
...
\lstinputlisting[caption={Ukázka formátování jednotek}, language={JavaScript}]{SourceCodes/Units.ts}

Všechny číselné hodnoty se automaticky zaokrouhlují s~rozumnou přesností:

\lstinputlisting[caption={Ukázka zaokrouhlování hodnot}, language={JavaScript}]{SourceCodes/Round.ts}
...
\lstinputlisting[caption={Tvorba vlastních jednotek}, language={JavaScript}]{SourceCodes/CreateUnits.ts}
...
\lstinputlisting[caption={Příklad použití komponenty EventsArea}, language={JavaScript}]{SourceCodes/EventsArea.ts}
