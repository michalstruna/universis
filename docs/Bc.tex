...
Vedle simulací je možné se setkat i~s~emulacemi. Podstatou simulace je nahrazení zkoumaného dynamického systému jeho simulujícím systémem za účelem zjistit o~původním systému nějaké informace.~\cite{kepler} U~emulace jsou naopak všechny informace o původním systému známé. Emulující systém pouze umožní práci s~původním systémem, který nemusí být k~dostupný.

Podle těchto vlastností lze na aplikaci, jež je součástí této práce, nahlížet jako na simulátor i~emulátor. Uživatel může využít simulaci těles pro to, aby se mimo jiné dozvěděl vzájemnou polohu či rychlost v určitém čase.
...
Jedná se o~web, který umožňuje zobrazit libovolné těleso v~databázi libovolný čas a~to z~libovolného jiného tělesa v~databázi. Obsahuje pouze tělesa z~naší sluneční soustavy. Umožňuje nastavit i~velikost zorného pole ve stupních. Vygenerovaný pohled je statický a~nelze s~ním už dál nijak interagovat.~\cite{nasasimulator}
...
Orbita je objekt s geometrií \textit{THREE.BufferGeometry}, jejíž tvar je vypočten na základě nejmenší (pericentrum) a~největší (apocentrum) vzdálenosti tělesa od těžiště  a~výstřednosti dráhy (excentricita). Jako materiál je použit \textit{THREE.LineMaterial}.
...
\item Pericentrum se vždy nachází přímo na opačné straně elipsy, než apocentrum. Pokud je oběžná dráha příliš excentrická, může se pozice skutečně nejbližšího bodu k~těžišti dráhy a~pericentra v~aplikaci lišit.
...
\begin{center}
\textit{S – plocha oběžné dráhy, T – perioda oběhu, d – vzdálenost od tělesa, d\textsubscript{max} – apocentrum, d\textsubscript{min} – pericentrum, a – velká poloosa, b – malá poloosa, e – excentricita}
\end{center}
...
\bibitem{nasasimulator}
Solar System Simulator. \textit{Jet propulsion laboratory.} [online]. [cit.\,22.~2.~2019]. Dostupné z: {\ttfamily \url{https://space.jpl.nasa.gov/}}.
